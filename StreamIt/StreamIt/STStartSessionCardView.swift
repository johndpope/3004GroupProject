...
class STStartSessionCardView: STGenericCardView {
	
	var startButton: UIButton!
	var controller: STInitialController!
	
	init(title: String, controller: STInitialController) {
	...
	func generateStartButton() -> UIButton {
		...
		butt.addTarget(self, action: #selector(STStartSessionCardView.startServerPressed(_:)), forControlEvents: .TouchUpInside)
